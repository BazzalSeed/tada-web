...
- **`npm test`** — unit tests (`*.test.ts`). Pure, fast, no database or Docker.
- **`npm run test:integration`** — DB integration tests (`*.integration.test.ts`), Docker required. A Vitest `globalSetup` spins up a throwaway Postgres container, applies the migrations, runs the suite against it, then tears it down — fully isolated from your dev and prod data.
...
- **How it's built:** [docs/architecture.md](docs/architecture.md) — architecture, repo layout, the `app/api` surface.
