...
Most tests are pure and run on every `npm test` (no database needed). The four DB integration tests (`db`, `invite`, `quota`, `store`) need a real Postgres, so they're **skipped by default**. Run them with **`npm run test:db`** (Docker required): a Vitest `globalSetup` spins up a throwaway Postgres container, applies the migrations, runs the suite against it, then tears it down — fully isolated from your dev and prod data.
...
- Other scripts: `npm run build`, `npm run typecheck`, `npm run lint`, `npm test` (pure tests), `npm run test:db` (DB integration tests in a throwaway container). The marketing landing previews standalone via `python3 design/landing-preview/.nocache_server.py`.
