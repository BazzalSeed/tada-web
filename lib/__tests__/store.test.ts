...
import { PrismaTadaStore, toNullableDate } from "@/lib/store";
import type { ActionPayload, RecurrenceRule } from "@/lib/contracts";

// Offline — guards the date-coercion boundary (FIX7 dated-capture regression):
// an unparseable date string must coerce to null, not an Invalid Date that
// Prisma rejects mid-persist (which collapsed dated captures to "Screenshot
// capture" under the capture-first fallback).
